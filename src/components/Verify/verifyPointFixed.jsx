...
class VerifyPointFixed extends Component {
  ...
  // 初始化数据
  getData() {
    getPicture({ captchaType: this.state.captchaType, clientUid: localStorage.getItem('point'), ts: Date.now() }).then(
      (res) => {
        if (res.repCode === '0000') {
          this.setState({
      ...
      if (this.state.num === this.state.checkNum) {
        ...
        const data = {
          ...
          pointJson: this.state.secretKey
            ? aesEncrypt(JSON.stringify(this.state.tempPoints), this.state.secretKey)
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: '50%',
                          position: 'absolute',
                          top: `${parseInt(tempPoint.y - 10, 10)}px`,
                          left: `${parseInt(tempPoint.x - 10, 10)}px`,
                          overflow: 'hidden',
                        }}
                      >
                        {index + 1}
